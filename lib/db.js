...
export async function setPaymentVerificado(id, verificado, voucherCode = null) {
  await initDb();
  applySqliteMigrations();
  const n = Number(id);
  ...
  if (DATABASE_URL) {
    const [row] = await sqlPg`
      UPDATE payments SET verificado = ${v}, voucher_code = ${vc}
      WHERE id = ${n}
      RETURNING id, registro_token, verificado, voucher_code
    `;
    return rowFromPg(row);
  }
  sqlite
    .prepare("UPDATE payments SET verificado = ?, voucher_code = ? WHERE id = ?")
    .run(v ? 1 : 0, vc, n);
  return sqlite
    .prepare("SELECT id, registro_token, verificado, voucher_code FROM payments WHERE id = ?")
    .get(n);
}
...
export async function listPayments() {
  ...
  if (DATABASE_URL) {
    const rows = await sqlPg`
      SELECT id, registro_token, voucher_code, nombre, email, telefono, metodo, monto, creditos, verificado, referencia, notas, mp_preference_id, mp_payment_id, mp_status, comprobante_path, created_at
      FROM payments
